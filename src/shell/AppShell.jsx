...
// Founder-only build map, SELF-RETIRING in production: renders only on
// dev hosts (netlify.app previews / localhost). The prod domain never
// matches, so the tab vanishes there with zero config (Brice, 8/5/26).
const isDevHost = /(\.netlify\.app)$|^localhost$|^127\./.test(window.location.hostname);
...
export default function AppShell({ session, memberships, activeOrgId, onSelectOrg }) {
  ...
  // Platform staff is an identity fact, not an org fact — checked on its own,
  // so a platform owner with zero tenant seats still gets Mission Control.
  ...
  useEffect(() => {
    ...
    supabase.from('platform_staff').select('role').eq('person_id', personId).maybeSingle()
      .then(({ data }) => { if (!cancelled) setIsStaff(!!data); });
    ...
    return (
      <div style={{ height: '100vh', display: 'flex', overflow: 'hidden' }}>
        <aside style={{ width: 220, flexShrink: 0, background: 'var(--navy)', display: 'flex', flexDirection: 'column', padding: '18px 14px' }}>
          ...
          <div style={{ marginBottom: 16, padding: '8px 10px', borderRadius: 8, background: 'var(--navy-soft)', color: 'var(--gold)', fontSize: 12, fontWeight: 700 }}>
            Platform staff
          </div>
          <nav style={{ flex: 1 }}>
            <NavLink to="/mission-control" style={navLink} data-testid="nav-mission-control">Mission Control</NavLink>
          </nav>
    ...
    <div style={{ height: '100vh', display: 'flex', overflow: 'hidden' }}>
      ...
      <aside style={{ width: 220, flexShrink: 0, background: 'var(--navy)', display: 'flex', flexDirection: 'column', padding: '18px 14px' }}>
        ...
        <nav style={{ flex: 1 }}>
          ...
          {isDevHost && active?.role === 'owner' && (
            <NavLink to="/progress" style={navLink} data-testid="nav-progress">Build Progress</NavLink>
          )}
      ...
      <main style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <Routes>
          ...
          <Route path="/progress" element={isDevHost && active?.role === 'owner' ? <Progress /> : <Navigate to="/" replace />} />
